...
class AffectionService:
    ...
    async def get_relationship(self, user_id: int) -> dict:
        """获取用户与 Hiyori 的关系"""
        if not self.pool:
            logger.warning("AffectionService.get_relationship: pool is None, returning defaults")
    ...
    async def update_after_message(self, user_id: int, message: str) -> dict:
        """用户发消息后更新好感度,返回变化信息"""
        if not self.pool:
            logger.warning("AffectionService.update_after_message: pool is None, cannot update")
        ...
        try:
            ...
            async with self.pool.acquire() as conn:
                ...
                # 更新数据库
                # 先更新 level_changed_at（用旧值比较）
                if new_level != old_level:
                    await conn.execute(
                        """UPDATE user_relationships
                           SET level_changed_at = CURRENT_TIMESTAMP
                           WHERE user_id = $1""",
                        user_id
                    )
                await conn.execute(
